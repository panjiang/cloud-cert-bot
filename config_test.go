...
func TestConfigComplete(t *testing.T) {
	t.Run("valid with default provider", func(t *testing.T) {
		...
		if cfg.Domains[0].Port != defaultTLSPort {
			t.Fatalf("Port = %d, want %d", cfg.Domains[0].Port, defaultTLSPort)
		}
	})
	...
	t.Run("domain custom port", func(t *testing.T) {
		cfg := Config{
			Alert:           AlertConfig{BeforeExpiredStr: "10d"},
			DefaultProvider: ProviderTencentCloud,
			ProviderConfigs: ProviderConfigs{
			...
			Domains: []DomainConfig{
				{Domain: "doc.yourdomain.com", Port: 5443, CertPath: "/etc/nginx/ssl/doc.crt", KeyPath: "/etc/nginx/ssl/doc.key"},
			},
		}
		...
		if cfg.Domains[0].Port != 5443 {
			t.Fatalf("Port = %d, want 5443", cfg.Domains[0].Port)
		}
	})

	t.Run("domain must not include port", func(t *testing.T) {
		cfg := Config{
			Alert:           AlertConfig{BeforeExpiredStr: "10d"},
			DefaultProvider: ProviderTencentCloud,
			ProviderConfigs: ProviderConfigs{
			...
			Domains: []DomainConfig{
				{Domain: "doc.yourdomain.com:5443", CertPath: "/etc/nginx/ssl/doc.crt", KeyPath: "/etc/nginx/ssl/doc.key"},
			},
		}

		if err := cfg.Complete(); err == nil {
			t.Fatal("Complete() expected domain-with-port error")
		}
	})

	t.Run("invalid domain ports", func(t *testing.T) {
		for _, tt := range []struct {
			port           int
			portConfigured bool
		}{
			{port: 0, portConfigured: true},
			{port: -1},
			{port: 65536},
		} {
			cfg := Config{
				...
				Domains: []DomainConfig{
					{Domain: "doc.yourdomain.com", Port: tt.port, CertPath: "/etc/nginx/ssl/doc.crt", KeyPath: "/etc/nginx/ssl/doc.key", portConfigured: tt.portConfigured},
				},
			}

			if err := cfg.Complete(); err == nil {
				t.Fatalf("Complete() port=%d expected error", tt.port)
			}
		}
	})

	t.Run("same domain with different ports is accepted", func(t *testing.T) {
		cfg := Config{
			Alert:           AlertConfig{BeforeExpiredStr: "10d"},
			DefaultProvider: ProviderTencentCloud,
			ProviderConfigs: ProviderConfigs{
			...
			Domains: []DomainConfig{
				{Domain: "doc.yourdomain.com", Port: 443, CertPath: "/etc/nginx/ssl/doc.crt", KeyPath: "/etc/nginx/ssl/doc.key"},
				{Domain: "doc.yourdomain.com", Port: 5443, CertPath: "/etc/nginx/ssl/doc-alt.crt", KeyPath: "/etc/nginx/ssl/doc-alt.key"},
			},
		}
...
`)
	...
	t.Run("rejects explicit zero port", func(t *testing.T) {
		path := filepath.Join(t.TempDir(), "config.yaml")
		data := []byte(`alert:
  beforeExpired: 14d
defaultProvider: tencentcloud
...
`)
		...
		if _, err := LoadConfig(path); err == nil {
			t.Fatal("LoadConfig() expected explicit zero port error")
		}
	})
}
