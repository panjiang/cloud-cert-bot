...
type probeCertificateFunc func(ctx context.Context, domain string, port int) (*ObservedCertificate, error)
type verifyDeploymentFunc func(ctx context.Context, domain string, port int, expectedFingerprint string) (*ObservedCertificate, error)
...
func (u *Updater) CleanupUnusedOldCertificates() error {
	...
	for _, domain := range u.cfg.Domains {
		...
		if err != nil {
			failures = append(failures, fmt.Sprintf("%s:%d: resolve provider: %v", domain.Domain, domain.effectivePort(), err))
			continue
		}

		live, err := u.probeCertificate(u.ctx, domain.Domain, domain.effectivePort())
		if err != nil {
			failures = append(failures, fmt.Sprintf("%s:%d: probe current certificate: %v", domain.Domain, domain.effectivePort(), err))
			continue
		}
		...
		if err := provider.CleanupUnusedOldCertificates(u.ctx, domain.Domain, toProviderObservedCertificate(live), providerpkg.CleanupOptions{
			...
			failures = append(failures, fmt.Sprintf("%s:%d: cleanup unused old certificates: %v", domain.Domain, domain.effectivePort(), err))
			continue
		}
	}
...
func (u *Updater) BuildCleanupPlan(cleanupUnused, cleanupExpired bool) ([]providerpkg.CleanupCandidate, error) {
	...
	if cleanupUnused {
		...
		for _, domain := range u.cfg.Domains {
			...
			if err != nil {
				failures = append(failures, fmt.Sprintf("%s:%d: list unused old certificate cleanup candidates: %v", domain.Domain, domain.effectivePort(), err))
				continue
			}
	...
	for _, domain := range u.cfg.Domains {
		...
		if !ok {
			...
			zap.L().Warn("stopping certificate check round because domain update failed",
				...
				zap.Int("port", domain.effectivePort()),
...
func (u *Updater) handleDomain(ctx context.Context, domain DomainConfig, options CheckOptions) (bool, bool) {
	...
	currentCert, err := u.probeCertificate(ctx, domain.Domain, domain.effectivePort())
	if err != nil {
...
func (u *Updater) verifyDeployedDomain(ctx context.Context, domain DomainConfig, nextCert *CertificateMaterial, result *DeployResult) (*ObservedCertificate, error) {
	...
	observed, err := u.verifyDeployment(ctx, domain.Domain, domain.effectivePort(), nextCert.Fingerprint)
	if err != nil {
...
func (u *Updater) managedDomains() []string {
	...
	seen := make(map[string]struct{}, len(u.cfg.Domains))
