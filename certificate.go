...
func probeTLSCertificate(ctx context.Context, domain string, port int) (*ObservedCertificate, error) {
	return probeTLSCertificateWithRootCAs(ctx, domain, port, nil)
}
...
func probeRawTLSCertificateWithConfig(ctx context.Context, domain string, port int, config *tls.Config) (*x509.Certificate, error) {
	...
	if !ok {
		return nil, fmt.Errorf("tls dial: unexpected connection type %T", conn)
	}
...
func browserCertificateError(err error) error {
	code := browserCertificateErrorCode(err)
	if code == "" {
		return err
	}
...
func browserCertificateErrorCode(err error) string {
	var verifyErr *tls.CertificateVerificationError
	if errors.As(err, &verifyErr) && verifyErr.Err != nil {
		if code := browserCertificateErrorCode(verifyErr.Err); code != "" {
			return code
		}
	}
...
func verifyExternalDeployment(ctx context.Context, domain string, port int, expectedFingerprint string) (*ObservedCertificate, error) {
	var last *ObservedCertificate
	var lastErr error

	for i := 0; i < 5; i++ {
		last, lastErr = probeTLSCertificate(ctx, domain, port)
