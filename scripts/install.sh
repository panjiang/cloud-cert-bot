...
REPO="${REPO:-panjiang/cloud-cert-bot}"
...
Restart=always
RestartSec=30s
RuntimeDirectory=cloud-cert-bot
...
echo "Run one normal check/update round before starting the service:"
echo "  sudo ${INSTALL_DIR}/${BINARY_NAME} -config=${CONFIG_DIR}/config.yaml -run-once"
echo "  Note: the update lock only applies while a normal update round is running."
