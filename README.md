...
```sh
curl -fsSL https://raw.githubusercontent.com/panjiang/cloud-cert-bot/main/scripts/install.sh | sudo sh
```

Optional: install a specific version instead of the latest release:

```sh
curl -fsSL https://raw.githubusercontent.com/panjiang/cloud-cert-bot/main/scripts/install.sh | \
...
```sh
curl -fsSL https://ghproxy.net/https://raw.githubusercontent.com/panjiang/cloud-cert-bot/main/scripts/install.sh | \
  sudo env GITHUB_PROXY=https://ghproxy.net VERSION=<release-tag> sh
```

## Configure

If you want to start from the installed example file and `/etc/cloud-cert-bot/config.yaml` does not already exist:

```sh
sudo cp -n /etc/cloud-cert-bot/config.yaml.example /etc/cloud-cert-bot/config.yaml
```
...
```sh
sudo vi /etc/cloud-cert-bot/config.yaml
```
...
domains:
  - domain: doc.yourdomain.com
    # Optional TLS port to probe and verify. Defaults to 443.
    port: 443
...
- For services on non-443 TLS ports, keep `domain` as the bare hostname and set `port`, for example `port: 5443`.
- Command entries are executed with `sh -lc`.
- Domain-level `postCommands` receive `{{.Domain}}`, `{{.Port}}`, `{{.CertPath}}`, `{{.KeyPath}}`, `{{.BackupCertPath}}`, and `{{.BackupKeyPath}}`.
...
```sh
sudo /usr/local/bin/cloud-cert-bot -config=/etc/cloud-cert-bot/config.yaml -run-once
```
...
```sh
sudo systemctl enable --now cloud-cert-bot
sudo systemctl status cloud-cert-bot
```

View service logs:

```sh
sudo journalctl -u cloud-cert-bot -f
```
...
```sh
sudo /usr/local/bin/cloud-cert-bot -config=/etc/cloud-cert-bot/config.yaml -cleanup-configured-old
```
...
```sh
sudo /usr/local/bin/cloud-cert-bot -config=/etc/cloud-cert-bot/config.yaml -cleanup-all-expired
```
...
```sh
curl -fsSL https://raw.githubusercontent.com/panjiang/cloud-cert-bot/main/scripts/install.sh | sudo sh
sudo systemctl restart cloud-cert-bot
```

## Uninstall

```sh
curl -fsSL https://raw.githubusercontent.com/panjiang/cloud-cert-bot/main/scripts/uninstall.sh | sudo sh
```

Keep the runtime config during uninstall:

```sh
curl -fsSL https://raw.githubusercontent.com/panjiang/cloud-cert-bot/main/scripts/uninstall.sh | \
