...
func TestProviderResolveCertificate(t *testing.T) {
	...
	t.Run("does not treat issued wait-issue certificate as pending", func(t *testing.T) {
		applied := false
		provider := &Provider{
			cfg: Config{
				AutoApply: AutoApplyConfig{
		...
		if !applied {
			t.Fatal("ApplyCertificate should be called when existing wait-issue certificate is already issued")
		}
		if resolution == nil || resolution.Pending == nil {
			t.Fatal("ResolveCertificate() expected pending certificate from new application")
		}
		if resolution.Pending.CertificateID != "new-applied-cert" {
			t.Fatalf("Pending.CertificateID = %q, want %q", resolution.Pending.CertificateID, "new-applied-cert")
		}
	})
