...
func TestUpdaterRunOnceSkipsProviderOutsideRenewalWindow(t *testing.T) {
	...
	updater := &Updater{
		...
		probeCertificate: func(ctx context.Context, domain string, port int) (*ObservedCertificate, error) {
		...
		verifyDeployment: func(ctx context.Context, domain string, port int, expectedFingerprint string) (*ObservedCertificate, error) {
...
func TestUpdaterRunOnceStopsAfterInitialProbeFailure(t *testing.T) {
	provider := &fakeUpdaterProvider{}
	deployer := &fakeUpdaterDeployer{}
	notifier := &fakeUpdaterNotifier{}
	updater := &Updater{
		cfg: &Config{
			...
			Domains: []DomainConfig{
				{
					Domain:            "harbor-cloud.houzzkit.com",
					EffectiveProvider: ProviderTencentCloud,
				},
			},
		},
		...
		ctx:       context.Background(),
		probeCertificate: func(ctx context.Context, domain string, port int) (*ObservedCertificate, error) {
			return nil, errors.New("tls dial: net::ERR_CERT_AUTHORITY_INVALID: x509: certificate signed by unknown authority")
		},
		verifyDeployment: func(ctx context.Context, domain string, port int, expectedFingerprint string) (*ObservedCertificate, error) {
			t.Fatal("verifyDeployment should not be called when current certificate probe fails")
			return nil, nil
		},
	}
	...
	if deployer.deployCalls != 0 {
		t.Fatalf("deploy calls = %d, want 0", deployer.deployCalls)
	}
...
func TestUpdaterRunOnceForceDeploysAndVerifies(t *testing.T) {
	...
	if provider.lastResolveDomain != "example.com" {
		t.Fatalf("ResolveCertificate domain = %q, want example.com", provider.lastResolveDomain)
	}
	if probePort != 5443 {
		t.Fatalf("probe port = %d, want 5443", probePort)
	}
	if verifyPort != 5443 {
		t.Fatalf("verify port = %d, want 5443", verifyPort)
	}
...
func TestUpdaterManagedDomainsDeduplicatesPorts(t *testing.T) {
	updater := &Updater{
		cfg: &Config{
			Domains: []DomainConfig{
				{Domain: "doc.example.com", Port: 443, EffectiveProvider: ProviderTencentCloud},
				{Domain: "doc.example.com", Port: 5443, EffectiveProvider: ProviderTencentCloud},
				{Domain: "api.example.com", Port: 443, EffectiveProvider: ProviderTencentCloud},
			},
		},
	}

	got := updater.managedDomains()
	want := []string{"doc.example.com", "api.example.com"}
	if len(got) != len(want) {
		t.Fatalf("managedDomains() = %#v, want %#v", got, want)
	}
	for i := range want {
		if got[i] != want[i] {
			t.Fatalf("managedDomains() = %#v, want %#v", got, want)
