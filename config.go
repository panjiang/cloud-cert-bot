...
func (d *DomainConfig) UnmarshalYAML(value *yaml.Node) error {
	type domainConfig DomainConfig
	var decoded domainConfig
	if err := value.Decode(&decoded); err != nil {
		return err
	}
	for i := 0; i+1 < len(value.Content); i += 2 {
		if value.Content[i].Value == "port" {
			decoded.portConfigured = true
			break
		}
	}
	*d = DomainConfig(decoded)
	return nil
}
...
func (c *Config) Complete() error {
	...
	for i := range c.Domains {
		...
		key := domainConfigKey(c.Domains[i])
		if _, exists := seenDomains[key]; exists {
			return fmt.Errorf("duplicate domain and port: %s:%d", c.Domains[i].Domain, c.Domains[i].Port)
		}
...
func (d *DomainConfig) complete(defaultProvider string) error {
	...
	if strings.Contains(d.Domain, ":") {
		return fmt.Errorf("domain must not include a port; use port instead")
	}
	if d.CertPath == "" {
	...
	if d.Port == 0 && !d.portConfigured {
		d.Port = defaultTLSPort
	}
...
func (d DomainConfig) effectivePort() int {
	if d.Port == 0 {
...
func domainConfigKey(d DomainConfig) string {
	return fmt.Sprintf("%s:%d", strings.ToLower(d.Domain), d.effectivePort())
}
