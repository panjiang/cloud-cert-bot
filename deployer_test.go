...
func TestLocalDeployerPostCommandTemplateIncludesPort(t *testing.T) {
	domainName := "harbor-cloud.houzzkit.com"
	material := testCertificateMaterial(t, domainName, "new-cert", time.Date(2027, 6, 2, 3, 4, 5, 0, time.UTC))

	root := t.TempDir()
	certPath := filepath.Join(root, "ssl", "harbor-cloud.houzzkit.com.crt")
	keyPath := filepath.Join(root, "private", "harbor-cloud.houzzkit.com.key")
	outputPath := filepath.Join(root, "command.out")
	...
	result, err := deployer.DeployDomain(context.Background(), DomainConfig{
		...
		CertPath:     certPath,
		KeyPath:      keyPath,
		PostCommands: []string{"printf '%s' '{{.Domain}}:{{.Port}}' > " + strconv.Quote(outputPath)},
	}, material)
	if err != nil {
		t.Fatalf("DeployDomain() error = %v", err)
	}
	if len(result.Commands) != 1 {
		t.Fatalf("Commands = %#v, want one rendered command", result.Commands)
	}
	assertFileContent(t, outputPath, []byte("harbor-cloud.houzzkit.com:5443"))
}
